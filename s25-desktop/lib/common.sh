...
# trap_errors — شبكة أمان: تُظهر سبب أي توقف مفاجئ بدل أن ينتهي السكربت بصمت.
#
# مع set -e يكفي أن تُعيد دالة قيمة غير صفرية ليتوقف السكربت المُنادي كاملاً بلا
# أي رسالة — وهذا ما جعل التثبيت يتوقف مرة بعد طباعة قائمة الحزم فقط. مع هذا
# الاعتراض يُطبع اسم السكربت ورقم السطر والأمر الفاشل، فيصبح كل توقف قابلاً
# للتشخيص. set -E ضرورية ليعمل الاعتراض داخل الدوال أيضاً.
trap_errors() {
    set -E
    trap 'S25_RC=$?; err "توقف $(basename "${BASH_SOURCE[0]}") عند السطر ${LINENO} — الأمر: ${BASH_COMMAND} (رمز الخروج $S25_RC)"' ERR
}
...
# apt_refresh — تحديث فهرس apt. لا يفشل أبداً بحيث يُنهي السكربت المُنادي:
# مع set -e فإن دالة تُعيد قيمة غير صفرية تُنهي السكربت كاملاً وبلا رسالة، وقد
# كان ذلك يُسقط تثبيت طبقة ويندوز صامتاً عند أي تعثّر شبكة عابر. الفهرس القديم
# غالباً يكفي للتثبيت، وإن لم يكفِ فـ apt_install سيفشل برسالة واضحة.
apt_refresh() {
    export DEBIAN_FRONTEND=noninteractive
    apt_proot_config
    retry 4 apt_update_once && return 0
    warn "تعذر تحديث فهرس apt — سنتابع بالفهرس الحالي
    إن فشل تثبيت الحزم بعد ذلك فالسبب شبكة أو مصدر معطوب:
      apt-get update    (لعرض الخطأ الكامل)"
    return 0
}
